...
typedef u_quad_t dtn_timeval_t;

/**
 * An infinite wait is a timeout of "-1", eg. 0xffffffffffffffff.
 */
#define DTN_TIMEOUT_INF ((dtn_timeval_t) 0xffffffffffffffffUL)
 ...
 * id where the bundle was received. The hops_to_live should be set
 * to zero if the application does not wish to specify it when
 * sending a bundle, in which case, the BPA's configured default
 * value will be used. The hops_to_live is set to zero when a
 * received bundle does not contain a hops to live value.
 */
